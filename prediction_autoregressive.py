...
def prediction_autoregressive(date_start, date_end, data_file, k, network_model, num_timesteps_in, num_timesteps_out):
    ...
    timestamps = int(num_timesteps_in/num_timesteps_out)
    ...
    for tensor in list_of_tensors:
        ...
        for j in range(timestamps):
            ...
            # Sposta il tensore di input a sinistra e aggiungi la nuova predizione come ultimo valore della feature
            if j < timestamps - 1:
                ...
                if num_timesteps_out == 1:
                    mod_tensor[:, SENSOR,
                    -num_timesteps_out] = y_hat.squeeze().detach().numpy()  # Aggiungi la predizione come ultimo valore della feature
                else:
                    mod_tensor[:, SENSOR,
                    -num_timesteps_out:] = y_hat.squeeze().detach().numpy()
...
if __name__ == "__main__":
    prediction_autoregressive('2024-01-25', '2024-02-25', 'data/complete_dataframe.csv', 0.05, 'model/model_24_4_1_256_100_2_100_w.pth', 24, 4)
